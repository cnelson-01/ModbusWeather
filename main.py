...
values = [
    ModbusFloat("TempSensorA(f)", 85, rr),
    ModbusFloat("TempSensorB(f)", 101, rr),
    ...
    try:
        if values[0].modified:
            ...
            for v in values:
                print(str(v))
            print('\n')
            r = requests.post("http://127.0.0.1:8000/logTempData/", data={'tempInF': values[0].value, 'name': 'tempA'})
            print(r.status_code, r.reason)
            r = requests.post("http://127.0.0.1:8000/logTempData/", data={'tempInF': values[1].value, 'name': 'tempB'})
            print(r.status_code, r.reason)
